...
module.exports = function (server) {
    ...
    initializeApplication = function () {
        ...
        if (fs.existsSync("./app/data/")) {
            fs.readdirSync("./app/data/").forEach(function (file) {
                if (
                    file.indexOf(".json") > 0 &&
                    file.indexOf(".json") + 5 == file.length
                ) {
                    ...
                    try {
                        ...
                        APPINDEX.CONTROLLERS[clsName] = generateController(
                            clsName,
                            jsonData
                        );
    ...
    registerRoutePath = async function (rPath, method, config) {
        ...
        if (rPath.substr(rPath.length - 1, 1) == "/")
            ...
            case "DATA":
                switch (method) {
                    ...
                    case "PUT":
                        ...
                        break;
                    case "DELETE":
                        server.delete(rPath, (req, res, next) => {
                    ...
                    default:
                        console.log(
                            "\x1b[31m%s\x1b[0m",
                            `Error registering Route Path - ${rPath}@${method}`
                        );
                        return res.send({
                            status: "ERROR",
                            ...
                            METHOD_PARAMS(
                                ...
                                function (data, msgObj, errObj) {
                                    ...
                                    if (errObj) {
                                        ...
                                        if (
                                            config.processor &&
                                            config.processor.length > 0 &&
                                            config.processor.split(".").length > 1
                                        ) {
                                            const processorObj = config.processor.split(".");
                                            if (
                                                APPINDEX.PROCESSORS[processorObj[0].toUpperCase()] &&
                                                typeof APPINDEX.PROCESSORS[
                                                processorObj[0].toUpperCase()
                                                ][processorObj[1]] == "function"
                                            ) {
                                                APPINDEX.PROCESSORS[processorObj[0].toUpperCase()][
                                                    processorObj[1]
                                                ](data, config, req, function (resultData) {
                                                    return res.send({
                                                        status: "success",
                                                        msg: msgObj,
                                                        data: resultData,
